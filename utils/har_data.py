...
def get_pamap(remove_bad=False):
  class_name_translator = {1: "lying", 2: "sitting", 3: "standing",
                           ...
                           0: "other"}
  df = pd.DataFrame(io.loadmat(
      '../Data/pamap/generated/pamap.mat')["data_pamap"])
...
def get_data(d_name, TEST_SIZE, delete_class_column=False, user_test_set_size=0,
  ...
  if user_test_set_size:
    ...
    for cl in classes:
      for person in persons:
        user_cl_indices = list(
            df[(df[person_column] == person) & (df[label_pos] == cl)].index)
        try:
  ...
  train_persons, test_persons = utils.random_split_list(
      list(persons), TEST_SIZE)
  print(
      f"Persons: {persons}, train: {sorted(train_persons)}, test: {sorted(test_persons)}")
