...
class FOSTER(BaseLearner):
    def __init__(self, args):
        super().__init__(args)
        self.args = args
        self._network = FOSTERNet(args["model_type"], False, args=args)
        self._snet = None
    ...
    def _feature_compression(self, train_loader, test_loader):
        self._snet = FOSTERNet(self.args["model_type"], False, args=self.args)
        ...
        for _, (_, inputs, targets) in enumerate(test_loader):
            ...
            predicts = torch.topk(
                outputs,
                k=self.topk if self.topk<=outputs.shape[1] else outputs.shape[1],
