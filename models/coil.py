...
class COIL(BaseLearner):
    def __init__(self, args):
        super().__init__(args)
        self._network = SimpleCosineIncrementalNet(args["model_type"], False,
                                                   args=args)
    ...
    def solving_ot(self):
        with torch.no_grad():
            if self._total_classes == self.data_manager.get_total_classnum():
                ...
                return None
            each_time_class_num = self.data_manager.get_task_size(self._cur_task+1)
    ...
    def solving_ot_to_old(self):
        ...
        Q_cost_matrix = (
            torch.cdist(
                next_period_class_means, former_class_means,
                p=self.args["norm_term"]
            )
            + EPSILON
        ...
        transformed_hat_W = torch.mm(
            T.T,
            F.normalize(self._network.fc.weight[-current_class_num:, :], p=2,
                        dim=1)
        )
        return transformed_hat_W * len(former_class_means) * self.calibration_term
    ...
    def incremental_train(self, data_manager):
        ...
        self._network.update_fc(self._total_classes,
                                self.nextperiod_initialization)
    ...
    def _train(self, train_loader, test_loader):
        ...
        if self.args["quantMethod"] == "fp134" or self.args["quantMethod"] == "fp130":
          ...
          optimizer = optim.SGD(
              self._network.parameters(), lr=self.args['lr'], momentum=0.9,
              weight_decay=5e-4
          )  # 1e-5
          scheduler = optim.lr_scheduler.MultiStepLR(
              optimizer=optimizer, milestones=self.args['milestones'],
    ...
    def _update_representation(self, train_loader, test_loader, optimizer,
        ...
        for _, epoch in enumerate(prog_bar):
            ...
            weight_ot_co_tuning = (epoch / self.args['epochs']) ** 2.0
        ...
        with torch.no_grad():
            for class_idx in range(low, high):
                try:
                    ...
                    print("class {} not found".format(class_idx))
        self._network.train()
