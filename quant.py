...
class Conv2d_track(nn.Conv2d):
  ...
  def forward(self, input):
    ...
    track_stats['wgts'][self.track_name] = (self.weight.mean(), self.weight.max(), self.weight.min() )
    track_stats['acts'][self.track_name] = (input.mean(), input.max(), input.min() )
...
class GradTrack(Function):
  ...
  def backward(ctx, grad_output):
    ...
    track_stats['grads'][ctx.name].append(grad_output.flatten()[torch.randint(size_total, ( int(size_total * .01) ,))].cpu())
    ...
    if self.quantizeFwd:
      if self.calibrate:
        if calibrate_phase:
          ...
          w_q = UniformQuantizeSawb_calib.apply(self.weight, self.QpW, self.QnW, scale_library['w'][self.uname]['scale_w'], scale_library['w'][self.uname]['clip_w'])
      else:
      ...
      if self.calibrate:
        if calibrate_phase:
          ...
          qinput = UniformQuantizeSawb_calib.apply(input, self.QpA, self.QnA, scale_library['a'][self.uname]['scale_a'], scale_library['a'][self.uname]['clip_a'])
      else:
